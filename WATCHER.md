...
Why: GitHub-hosted runners currently receive `HTTP Error 403: Forbidden` from Substack's archive API for `engineeringgenerosity.substack.com`. The local adapter run succeeds and passes the sitemap completeness gate. Pretending the GitHub Action works would be tidy and false, a common but unlovely combination.
